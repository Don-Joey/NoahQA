...
import itertools
from rgnet_base_zh.fields_test import SourceTextField, SourceQuestionTextField, TargetTextField
from rgnet_base_zh.custom_instance import SyncedFieldsInstance
...
from rgnet_base_zh.spacy_tokenizer import SpacyTokenizer

from rgnet_base_zh.util import (
    IGNORED_TOKENS,
    STRIPPED_CHARACTERS,
            ...
            for qa_number, question_answer in enumerate(passage_info["qa_pairs"]):
                ...
                if "answer" in question_answer:
                    ...
                    if question_answer["answer_type"] == [6, 1]:
                        ...
                        answer_txt = '未知'
                    answer[str(question_answer["answer_type"][0])] = str(question_answer["answer"]).rstrip('了').rstrip('。')#.lower()
                    if question_answer["answer_type"][0] == 6 or question_answer["answer_type"] == [2, 0] or \
                        ...
                        answer['spans'] = str(question_answer["answer"]).replace('了','').rstrip('。')#.lower()
                        answer_txt = str(question_answer["answer"]).replace('了','').rstrip('。')#str(question_answer["answer"])#.lower()
                    ...
                    if question_answer["answer_type"][0] == 3 or question_answer["answer_type"] == [4, 0]:
                        ...
                        try:
                            ...
                            answer_txt = ' '.join(split_equation(str(question_answer['answer_temp']), num_list))
                        except KeyError:
                            try:
                                ...
                                answer_txt = ' '.join(
                                    split_equation(str(question_answer['answer_formula']), num_list))
                            except KeyError:
                                ...
                                answer_txt = ' '.join(
                                    split_equation(str(question_answer['answer']), num_list))
                        try:
