...
class CopyMechanism(nn.Module):
    ...
    def forward(
        ...
        pre_pgen_tensor = torch.cat([context_vector, decoder_hidden_state, decoder_input], dim=-1)    #add_pgen

        pgen = self.pgen(pre_pgen_tensor)  # batchsizeXseqlenX1 #add_pgen
        #print("pgen:",pgen)
        pcopy = 1.0 - pgen   #add_pgen
        ...
        #copy_probabilities = (1e-7) * copy_probabilities
        copy_probabilities.scatter_add_(2, encoder_input, attn_weights)
        #print("copy_probabilities:",copy_probabilities)
        #total_probabilities = copy_probabilities
        total_probabilities=pgen * output_probabilities + pcopy * copy_probabilities  #origin
        #total_probabilities = copy_probabilities
        #print("total_probabilities",total_probabilities.size())
        #print(total_probabilities)
        #total_probabilities = self.m(total_probabilities)
        #total_probabilities = self.m(total_probabilities)
        #copy_probabilities = self.m(copy_probabilities)

        return total_probabilities  # batchXseqlenXoutvocab , batchsizeXseqlenX1
'''
class Attention(nn.Module):
    """ Simple Attention
