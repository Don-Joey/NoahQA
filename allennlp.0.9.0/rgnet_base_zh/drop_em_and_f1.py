...
from rgnet_base_zh.drop_eval import (get_metrics as drop_em_and_f1,
...
class DropEmAndF1(Metric):
    ...
    def metric_max_over_ground_truths(self, metric_fn, prediction, ground_truths, history_answer, answer_type,id_dict):
        scores_for_ground_truths = []
        for ground_truth in ground_truths:
            score = metric_fn(prediction, ground_truth, history_answer, answer_type,id_dict)
            scores_for_ground_truths.append(score)

        return max(scores_for_ground_truths)

    @overrides
    def __call__(self, prediction: Union[str, List], ground_truths: List, history_answer: Dict, answer_type: List, id_dict: Dict):  # type: ignore
        """
        Parameters
        ----------
