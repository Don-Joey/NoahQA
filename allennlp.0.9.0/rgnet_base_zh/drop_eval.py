...
from rgnet_base_zh.compute_tool import compute_gen_ans,split_equation_tt
from rgnet_base_zh.util import split_equation_evuation
import thulac
...
def get_metrics(predicted: Union[str, List[str], Tuple[str, ...]],
    ...
    if set(re.findall(r'<a\d>', gold[0][0])):
        ...
        for key,value in history_answer.items():
            if key in predicted:
                if set(re.findall(r'<a\d>',str(gold[0][0]).lower()))==set(re.findall(r'<a\d>',str(predicted).lower())) and set(re.findall(r'<a\d>',str(gold[0][0]))):
    ...
    if len(gold)==2:
        ...
        #print("gold_bags",f1_predicted_bags)
        #print("predicted_bags",f1_gold_bags)
        #print("*"*97)
    ...
    step_em = {
        'question_id': id_dict['question_id'],
        'passage_id': id_dict['passage_id'],
        'em': exact_match
    }
    with open("generation_cn.json", 'a', encoding='utf-8') as f:
        #json.dump(edge,f, ensure_ascii=False, indent=4)
        j = json.dumps(step_em, ensure_ascii=False)
        f.write(j + '\n')
        f.close()

    return exact_match, f1, supporting_exact, supporting_count,type_EM,type_f1,type_count
