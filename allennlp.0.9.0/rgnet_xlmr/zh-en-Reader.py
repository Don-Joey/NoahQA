...
AUGMENT = '未知 | 是 | 否 | 不知道 | unknown | yes | no | do not know | ( ) { } [ ] + - * / ^'
...
@Tokenizer.register("xlm-zh-en-edge")
...
@TokenIndexer.register("xlm-zh-en-edge")
...
@DatasetReader.register("zh_en_xlm-edge")
class ComwpReader(DatasetReader):
    ...
    def __init__(
        ...
        super().__init__(lazy)
        self._zh_tokenizer = tokenizer
        self._en_tokenizer = SpacyTokenizer()
    ...
    def _read(self, file_path: str):
        ...
        zh_file_path, en_file_path = file_list[0], file_list[1]
        
        logger.info("Reading file at %s", file_path)
        with open(en_file_path, 'rb') as en_dataset_file:
            en_dataset = json.load(en_dataset_file)
        with open(zh_file_path, 'rb') as zh_dataset_file:
            zh_dataset = json.load(zh_dataset_file)
        logger.info("Reading the dataset")
        kept_count, skip_count = 0, 0
        for passage_id, passage_info in zh_dataset.items():
            passage_info_list = ['<p>']
            ...
            passage_tokens = self._zh_tokenizer.tokenize(passage_text)
            ...
            for qa_number, question_answer in enumerate(passage_info["qa_pairs"]):
                ...
                if question_type[0] == 6 or question_type == [2, 0] or question_type[0] == 1 or question_type == [5, 0] or question_type[0] == 3 or question_type[0] == 4:
                    ...
                    instance,length_of_input = self.zh_text_to_instance(
                        question_text,
                        passage_text,
                        passage_length,
                        question_id,
                ...
                if instance is not None and length_of_input<=512:
                    ...
                    skip_count += 1
        for passage_id, passage_info in en_dataset.items():
            passage_info_list = ['<p>']
            for token in passage_info['passage']:
                if token[1]:
                    passage_info_list.append('<t>')
            ...
            # passage_text = for passage_info["passage"]  \mark
            passage_tokens = self._en_tokenizer.tokenize(passage_text)
            passage_tokens = split_tokens_by_hyphen(passage_tokens)
            ...
            for qa_number, question_answer in enumerate(passage_info["qa_pairs"]):
                ...
                if question_type[0] == 6 or question_type == [2, 0] or question_type[0] == 1 or question_type == [5, 0] or question_type[0] == 3 or question_type[0] == 4:
                    ...
                    # if question_type[0] != 1 and question_type[0] != 5 and question_type != [6,1] and question_type[0] != 2 and question_type[0] != 4\
                    #       and question_type[0] != 3:
                    instance,length_of_input = self.en_text_to_instance(
                        question_text,
                        passage_text,
                        passage_length,
                        question_id,
    ...
    def zh_text_to_instance(
        ...
        question_tokens = self._zh_tokenizer.tokenize(question_text)
        question_tokens = split_tokens_by_hyphen(question_tokens)


        question_tokens.insert(0, self._zh_tokenizer.tokenize('<q>')[0])
        ...
        if len(history) > 1:
            ...
            for i in range(1, len(history)):
                d = i#len(history) - i
                temp.append(self._zh_tokenizer.tokenize('<q{}>'.format(d))[0])
                temp.extend(split_tokens_by_hyphen(self._zh_tokenizer.tokenize(str(history[i - 1][0]).lower())))
                temp.append(self._zh_tokenizer.tokenize('<a{}>'.format(d))[0])
                temp.extend(split_tokens_by_hyphen(self._zh_tokenizer.tokenize(str(history[i - 1][1]).lower())))
                temp.append(self._zh_tokenizer.tokenize('</q{}>'.format(d))[0])
        ...
        if answer_type == 'generation':
            ...
            for answer_text in answer_texts:
                answer_tokens = self._zh_tokenizer.tokenize(str(answer_text).lower())
    ...
    def en_text_to_instance(
        ...
        question_tokens = self._en_tokenizer.tokenize(question_text)
        question_tokens = split_tokens_by_hyphen(question_tokens)


        question_tokens.insert(0, self._en_tokenizer.tokenize('<q>')[0])
        question_text = '<q> ' + question_text
        history_answer = {}
        question_evidence_nodes = []
        if len(history) > 1:
            ...
            for i in range(1, len(history)):
                d = i#len(history) - i
                temp.append(self._en_tokenizer.tokenize('<q{}>'.format(d))[0])
                temp.extend(split_tokens_by_hyphen(self._en_tokenizer.tokenize(str(history[i - 1][0]).lower())))
                temp.append(self._en_tokenizer.tokenize('<a{}>'.format(d))[0])
                temp.extend(split_tokens_by_hyphen(self._en_tokenizer.tokenize(str(history[i - 1][1]).lower())))
                temp.append(self._en_tokenizer.tokenize('</q{}>'.format(d))[0])
        ...
        if answer_type == 'generation':
            ...
            #print("answer_texts:",answer_texts)
            for answer_text in answer_texts:
                answer_tokens = self._en_tokenizer.tokenize(str(answer_text).lower())
                answer_tokens = split_tokens_by_hyphen(answer_tokens)
                #print("answer_tokens",answer_tokens)
                #answer_tokens = split_tokens_by_hyphen(answer_tokens)
                answer_tokens_replace = []
