...
from rgnet_xlmr.drop_em_and_f1 import DropEmAndF1
#from naqanet_edge_gated_roberta.pointernet import Decoder
from allennlp.data.dataset import Batch
from allennlp.common.util import START_SYMBOL, END_SYMBOL

from rgnet_xlmr.CopyDecoder import Attention,CopyMechanism
from rgnet_xlmr.compute_tool import compute_gen_ans
from rgnet_xlmr.gnn import CharmGNN

from transformers import RobertaTokenizer,RobertaModel,RobertaConfig, XLMRobertaTokenizer, XLMRobertaModel, XLMRobertaConfig
...
class NumericallyAugmentedQaNet(Model):
    ...
    def __init__(self, vocab: Vocabulary,
        ...
        self._bert_tokenizer = XLMRobertaTokenizer.from_pretrained(roberta_pretrained_model)
        self.vocab_size = self._bert_tokenizer.vocab_size
        self.BERT = XLMRobertaModel.from_pretrained(roberta_pretrained_model)
        Roberta_Config = XLMRobertaConfig.from_pretrained(roberta_pretrained_model)
        ...
        if "generation" in self.answering_abilities:
            #self.embedded = RobertaEmbeddings(Roberta_Config)
            self.embedded = self.BERT.embeddings
    ...
    def forward(self,  # type: ignore
        ...
        bert_out,_ = self.BERT(input_ids=question_passage_tokens,attention_mask =pad_mask)#, token_type_ids=seqlen_ids)
        ...
        q_node_start_mask = (q_node_start_indices > -1).long()
        ...
        q_node_end_mask = (q_node_end_indices > -1).long()
        ...
        question_node_mask = (question_node_indice > -1).long()
        ...
        p_node_start_mask = (p_node_start_indices > -1).long()
        ...
        p_node_end_mask = (p_node_end_indices > -1).long()
        ...
        if "generation" in self.answering_abilities:
            ...
            best_generation_log_probs = util.replace_masked_values(best_generation_log_probs, answer_as_generation['mask'][:,1:], -1e7)
        ...
        if metadata is not None:
            ...
            for i in range(batch_size):
                ...
                answer_nodes = metadata[i].get("evidence_nodes", [])
                predicted_answer = str(predicted_answer).replace(' ','')
                predicted_answer = str(predicted_answer).replace('▁',' ').strip()
                ...
                #    answer_annotations[0]['spans'][0] = str(answer_annotations[0]['spans'][0]).replace(' ','').replace('▁',' ').strip()
                '''
                print("predicted_answer",predicted_answer)
    ...
    def get_metrics(self, reset: bool = False) -> Dict[str, float]:
        exact_match, f1_score, supporting_ratio, supporting_total, judge_em, comp_em, open_em, unsure_em, extract_em, arithmetic_em, arithmetic_open_em, judge_f1, comp_f1, open_f1, unsure_f1, extract_f1, arithmetic_f1, arithmetic_open_f1 = self._drop_metrics.get_metric(
            reset)
