...
from rgnet_zh.utils.util import (
    IGNORED_TOKENS,
    STRIPPED_CHARACTERS,
...
class ComwpReader(DatasetReader):
        ...
        for passage_id, passage_info in dataset.items():
            ...
            for token in passage_info['passage']:
                if token[1]:
                    ...
                    passage_info_list.append('</t>')
                    node_dict['<p{}>'.format(node_count)] = node_count + 1
            ...
            for qa_number, question_answer in enumerate(passage_info["qa_pairs"]):
                ...
                if qa_number != 0:
                    node_count += 1
                    node_dict['<q{}>'.format(qa_number)] = node_count + 1
                ...
                if "answer" in question_answer:
                    ...
                    if question_answer["answer_type"] == [6, 1]:
                        ...
                        answer_txt = '未知'
                    answer[str(question_answer["answer_type"][0])] = str(question_answer["answer"]).rstrip('了').rstrip('。')
                ...
                edges = []
                for e in edge_list:
                    if e[1] < 0.2 and e[1] > 0.1:
                        edges.append(['<q{}>'.format(int(e[0])), '<p{}>'.format(int(e[1] * 100 - 1))])
                    elif e[1] >= 1:
                        edges.append(['<q{}>'.format(int(e[0])), '<q{}>'.format(int(e[1]))])
                    else:
                        edges.append(['<q{}>'.format(int(e[0])), '<p{}>'.format(int(e[1] * 10))])
                edges = [['<q>', e[1]] if e[0] == '<q{}>'.format(int(qa_number + 1)) else e for e in
                         edges]
    ...
    def text_to_instance(
        ...
        question_passage_tokens = [Token('[CLS]',idx=0,text_id=0)] + [Token(token.text,idx=(token.idx+6),text_id=(token.text_id+1)) for token in question_tokens]
        end_question_token = question_passage_tokens[-1]
        question_passage_tokens += [Token('[SEP]',idx=(end_question_token.idx+len(end_question_token.text)+1),text_id=(end_question_token.text_id+1))]
        end_question_token = question_passage_tokens[-1]
    ...
    def make_marginal_drop_instance(
        ...
        mask_test_indices_fields: List[Field] = [IndexField(index, question_passage_field) for index in mask_test]
        ...
        answer_tokens = [Token('[CLS]',idx=0,text_id=0)] + [Token(token.text,idx=token.idx+5,text_id=token.text_id+1) for token in answer_tokens]
        ...
        fields['mask_test_indices'] = ListField(mask_test_indices_fields)
        # print(metadata)
